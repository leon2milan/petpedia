...
def create_model_for_provider(path: str,
                              provider_to_use: str) -> InferenceSession:
    options = SessionOptions()
    options.graph_optimization_level = GraphOptimizationLevel.ORT_ENABLE_ALL
    if type(provider_to_use
            ) == list and provider_to_use == "CPUExecutionProvider":
...
def tokenize(dummy_text: List[str], tokenizer: PreTrainedTokenizer,
             type: TensorType) -> BatchEncoding:
    return tokenizer(
        text=dummy_text,
...
def prepare_input(
    ...
    inputs_pytorch = tokenize(dummy_text=[dummy_text] * 1,
                              tokenizer=tokenizer,
                              type=TensorType.PYTORCH)
    inputs_pytorch = {
        k: v.to(device="cuda")
...
def convert_to_onnx(model_pytorch: PreTrainedModel, output_path: str,
                    inputs_pytorch: Dict[str, torch.Tensor]) -> None:
    with torch.no_grad():
        torch.onnx.export(
...
def optimize_onnx(onnx_path: str, onnx_optim_fp16_path: str,
                  use_cuda: bool) -> None:
