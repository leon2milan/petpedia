...
class Calibrator(trt.IInt8Calibrator):
    def __init__(self):
        ...
        input_list: List[ndarray] = [
            np.zeros((32, 512), dtype=np.int32) for _ in range(3)
        ]
        # allocate GPU memory for input tensors
        self.device_inputs: List[DeviceAllocation] = [
            cuda.mem_alloc(tensor.nbytes) for tensor in input_list
        ]
...
def fix_fp16_network(
        network_definition: INetworkDefinition) -> INetworkDefinition:
...
def build_engine(
    ...
    with trt.Builder(logger) as builder:  # type: Builder
        with builder.create_network(
            ...
            with trt.OnnxParser(network_definition,
                                logger) as parser:  # type: OnnxParser
                ...
                engine: ICudaEngine = runtime.deserialize_cuda_engine(
                    trt_engine)
...
def infer_tensorrt(
    ...
    input_list: List[ndarray] = [
        tensor.astype(np.int32) for tensor in host_inputs.values()
    ]
    # allocate GPU memory for input tensors
