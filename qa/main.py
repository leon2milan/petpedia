...
# > The `Search` class is a wrapper for the `AdvancedSearch` class, which is a wrapper for the `Index`
# class
class Search(object):
    __slots__ = ['cfg', 'AS']
    ...
    def search(self, query):
        """
        It takes a query, searches the index, and returns a list of dictionaries containing the index,
        score, and doc for each item in the result
        ...
        if self.cfg.RETRIEVAL.TIME_PERFORMENCE:
            logger.info(f'Search takes: {time.time() - s}')
        return [{
            k: v
...
if __name__ == "__main__":
    ...
    test = [
        ...
        '我想养个狗，应该注意什么？', '我想养个猫，应该注意什么？', '藏獒得怎么样训练才会“认主人”', '什么是狗狗翻肠子？'
    ]
    ...
    for i in test:
        ...
        logger.info(f'search takes time: {time.time() - start}')
