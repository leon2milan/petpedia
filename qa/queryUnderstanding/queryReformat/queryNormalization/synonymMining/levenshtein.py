...
class Levenshtein_model():
    def __init__(self,
        ...
        self.output_path = os.path.join(base_path, 'Levenshtein_model_synonym.txt')
    ...
    def calculate_similarity(self, w1, w2):
        leven_dis = LexicalSimilarity.levenshteinDistance(w1, w2)
        leven_py_dis = 0.0
        if self.if_use_pinyin:
            py1 = get_pinyin(w1)
            py2 = self.can_py_dict[w2]
            leven_py_dis = LexicalSimilarity.levenshteinDistance(py1, py2)
        similarity = (1 - self.pinyin_weight
                      ) * leven_dis + self.pinyin_weight * leven_py_dis
        return similarity
    ...
    def synonym_detect(self, base_word, lock, input_word_code_dict):
        ...
        for word in base_word:
            cnt += 1
            logger.info(
                'process {b}, calculating the {a} base word......'.format(
                    a=cnt, b=os.getpid()))
            dis_dict = {}
            for candi in self.candidate_word:
                dis = self.calculate_similarity(word, candi)
                dis_dict[candi] = dis
            synonyms = sorted(dis_dict.items(),
                              key=lambda e: e[1],
                              reverse=True)
        ...
        for word in word_synonym.keys():
            ...
            synonyms = str(word_code) + '\t' + word + '\t' + '|'.join(synonyms)
            line += synonyms + '\n'
        logger.info('process {a} starting writing to file ......'.format(
            a=os.getpid()))
        with lock:
    ...
    def multipro_synonym_detect(self, input_word_code_dict):
        ...
        if word_num < self.processe_number:
            logger.info(
                'error, process number more than the amount of base word!')
            return
        for i in range(self.processe_number):
            if end > word_num: break
            word_id = self.base_word[start:end]
            p = Process(target=self.synonym_detect,
                        args=(word_id, lock, input_word_code_dict))
            pro_list.append(p)
            p.start()
