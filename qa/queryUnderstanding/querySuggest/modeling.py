...
from qa.queryUnderstanding.querySuggest import optimization_adafactor
from qa.queryUnderstanding.querySuggest.utils import get_assignment_map_from_checkpoint, get_shape_list, get_attention_mask, gelu, layer_norm, dropout, \
...
def mask_attention_for_ltr(attention_scores, attention_mask):
    ...
    return attention_scores * mask - tf.cast(
        1e10, attention_scores.dtype) * (1 - mask)
...
class GroverModel(object):
    ...
    def log_probs(self):
        ...
        return tf.reshape(logprobs_flat,
    ...
    def lm_loss(self):
        ...
        label_weights = tf.cast(tf.not_equal(target_ids_flat,
                                             self.pad_token_id),
        ...
        per_example_loss = -tf.reduce_sum(logprobs_flat * one_hot_labels,
                                          axis=[-1])
    ...
    def pooled_output(self, clf_token):
        ...
        """
        pool_idx = tf.cast(
            tf.argmax(tf.cast(tf.equal(self.input_ids, clf_token), tf.float32),
                      1), tf.int32)
        return tf.gather(
            self.hidden_state,
            tf.range(self.batch_size, dtype=tf.int32) * self.seq_length +
            pool_idx)
