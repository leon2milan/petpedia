...
if __name__ == '__main__':
    ...
    disease = list(specialize['疾病'].keys())
    symptom = list(specialize['症状'].keys())
    dog = list(specialize['犬'].keys())
    cat = list(specialize['猫'].keys())
    name2class = {n: c for c, b in specialize.items() for n, _ in b.items()}
    name2alias = reduce(lambda a, b: dict(a, **b), specialize.values())
    alias2name = {j if j else k: k for k, v in name2alias.items() for j in v}
    all_word = list(
        set([y for x in [[k, v] for k, v in alias2name.items()] for y in x]))
    for i in all_word:
        ah.add_word(i)
    ah.make()
    string = '哈士奇拆家怎么办'
    s = time.time()
    ah.search_all(string)
    print(time.time() - s)
