...
class SIMCSE:
        ...
        with torch.no_grad():
            if squeeze_flag:
                ...
                source_token_type_ids = source['token_type_ids'].to(
                    self.device)
                source_pred = self.model(source_input_ids,
                                         source_attention_mask,
                                         source_token_type_ids)

            else:
                # source        [batch, 1, seq_len] -> [batch, seq_len]
                source_input_ids = source['input_ids'].squeeze(1).to(
                    self.device)
    ...
    def query_sim(self, query: str, candidate_list: list):
        ...
        return F.cosine_similarity(query, candidate_list,
...
if __name__ == '__main__':
    ...
    test = [('哈士奇拆家吗', 0.7548817), ('哈士奇会拆家吗', 0.64450085),
            ('哈士奇为什么喜欢拆家', 0.5734687), ('萨摩耶拆家吗', 0.5666889),
            ('雪纳瑞拆家吗', 0.4354), ('为什么哈士奇喜欢拆家', 0.3191795),
            ('买宠物去宠物店OR家庭认养？', 0.3084151), ('柯基拆家吗', 0.24455209),
            ('拆家狂魔——哈士奇', 0.23657143), ('有不拆家的哈士奇吗', 0.20618913)]
    ...
    query = '犬瘟热'
    candidate = [('犬瘟热转阴', 0.88323), ('犬瘟热咳喘', 0.78485936),
                 ('犬瘟热怎么得的', 0.6642784), ('犬瘟热血常规', 0.6492873),
                 ('犬瘟热尿血', 0.6390894), ('犬瘟热是什么,犬瘟热症状有哪些', 0.6343337),
                 ('犬瘟热初期症状,犬瘟热怎么治', 0.62487304), ('犬瘟热后遗症有哪些', 0.61606777),
                 ('犬瘟热临床症状有哪些,犬瘟热症状', 0.60905147), ('呕泻犬瘟热', 0.59413147)]
    ...
    s = time.time()
    print(
        f'score: {simcse.query_sim(query, candidate_list)}, time: {time.time() - s}'
    )
