...
class PathExample():
    def __init__(self, cypher, sub_template='1a'):
        ...
        if sub_template == '1aa':
            ...
            self.path = cypher['r1.name'] + '\t' + cypher['r2.name'] + cypher[
                'm.name']
...
def sparql2cypher(sparql):
    entity = []
    # print("spaeql: ", sparql)
    target = re.findall(r"(select.+?where)",
                        sparql.lower())[0].replace('select',
                                                   '').replace('where',
                                                               '').strip()
    # print("target: ", target)
    ...
    for i in range(len(sql)):
        if i in [1, 3, 4, 6, 7, 9
                 ] and sql[i] != '<target>' and sql[i] != '<label>':
...
class Template():
    def __init__(self, cfg, templates=['1a', '2a', '2d']):
        self.cfg = cfg
        self.templates = templates
        self.neo4j = NEO4J(cfg)

    def get_cyphers(self, entity):
        cyphers = []
        for template in self.templates:
            if template == '1a':
                cypher_1aa = "match(n)-[r]->(m) \
                            where n.name ='{}' \
                ...
                cypher_2aa = "match(n)-[r1]->(q)-[r2]->(m) \
                            where n.name ='{}'  and r1.name<>'标签' and r2.name<>'标签' and r2.name<>'描述'\
                            return n.name, r1.name, m.name, r2.name, q.name".format(
                    entity)
                cypher_2ab = "match(n)-[r1]->(p)<-[r3]-(q)-[r2]->(m)  \
                            where n.name ='{}' and r3.name='中文名' and r1.name<>'标签' and r2.name<>'标签' and r2.name<>'描述'\
                            return n.name, r1.name, m.name, r2.name, q.name".format(
                    entity)
                cypher_2ac = "match(n)<-[r3]-(q)-[r1]->(q)-[r2]->(m) \
                            where n.name ='{}' and r3.name='中文名' and r1.name<>'标签' and r2.name<>'标签' and r2.name<>'描述'\
                            return n.name, r1.name, m.name, r2.name, q.name".format(
                    entity)
                cyphers.append({
    ...
    def get_graph_template(self, entitys, used_template=['1a', '2a', '2d']):
        result = []
        template_cypher = Template(used_template)
        for entity in entitys:
            cyphers = template_cypher.get_cyphers(entity['candiate_entity'])
            for cypher in cyphers:
                cypher_result = self.graph.run(cypher['cypher']).data()
...
if __name__ == "__main__":
    ...
    entitys = [{
        'candiate_entity': '湖上草',
        'score': 0.2635486125946045,
        'mention': '湖上草'
    }]
    temp = Template(cfg)
    result = temp.get_graph_template(entitys)
    print(len(result))
    print(result[:4])
